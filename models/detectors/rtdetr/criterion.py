...
class Criterion(nn.Module):
    def __init__(self, cfg, num_classes=80):
        ...
        self.gamma = 2.0
        # ------------ Matcher ------------
        self.matcher = HungarianMatcher(cfg['matcher_hpy'], alpha=0.25, gamma=2.0)
        # ------------- Loss weight -------------
        ## Main loss weight
        ...
        aux_weight_dict = {}
        dn_weight_dict = {}
        for i in range(cfg['de_num_layers']):
            aux_weight_dict.update({k + f"_aux_{i}": v for k, v in weight_dict.items()})
            dn_weight_dict.update({k + f"_dn_{i}": v for k, v in weight_dict.items()})
